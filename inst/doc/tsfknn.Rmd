...
author: "Francisco Martinez, Maria P. Frias, Maria D. Perez-Godoy, Antonio J. Rivera"
date: "`r Sys.Date()`"
output: 
  html_document:
    number_sections: yes
...
regression is described. The package allows, with only a function, to specify
...
more prominent machine learning technique used in time series forecasting,
other approaches, such as Gaussian Processess or KNN, have also been applied.
...
explained. Section 4 discusses some additional feature of our package and
...
prediction. As we have seen above, your can use the function `knn_examples` to see the examples associated with the model. You can also consult the predicition or get a plot through the `knnForecast` object:
...
next future value of the time series are the two last values of the time series (vector 
...
By the way, this artificial example of a time series with a constant lineal trend
illustrates the fact that KNN is not suitable for prediciting time series with a global
...
same graph. Because we are working with a monthly time series, we have thought that lags 1-12 are a suitable choice for selecting the features of the examples. In this case, the 12 last values of the time series are the new
...
In this section several additional features of our packaged are described.

## Combination and distance function

By default, the targets of the different nearest neighbors are averaged. However, it is possible to combine the targets using other aggregation functions. Currently, our package allows to choose among the mean and the 
median function using the `cb` parameter of the `knn_forecasting` function.
...
model combination in time series forecating. This way, the use of a time consuming optimization tool is avoided and the forecasts are not based on an unique, heuristic *k* value. Our package allows the use of this strategy 
specifying a vector of *k* values:
...
Sometimes a great number of time series have to be forecast. In that situation, an automatic way of generating the forecasts it is very useful. Our package is able to automatically choose *k* and the lags used as features. *k* is selected
as the square root of the number of examples. The lags are selected as 1:*f*, where *f* es the number of periods in the time series, for example, 4 for quarterly data.



# Conclusions

In R, just a few packages apply computational intelligence regression methods to time series forecasting. In this paper we have presented the **tsfknn** package that allows to forecast a time series using KNN regression. The interface of the
package is quite simple, with only a function the user can specify a KNN model
and predict a time series. Furthermore, several graphs can be generated illustrating how the prediction has been computed.
